...
class SubscribeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        tabBarItem = UITabBarItem(title: "訂閱", image: UIImage(named: "Subscribe.png"), tag: 1)
    }
        ...
        DispatchQueue.main.async() {
            if(connectionState == .Connected) {
                ...
                if(stateSubscribed) {
                    ...
                    self.subscribeButton.setTitle("取消訂閱", for: .normal)
                    ...
                    self.subscribeButton.setTitle("訂閱", for: .normal)
                ...
                self.subscribeQosSelector.isEnabled = false
                self.topicTextField.isEnabled = false
                self.subscribeButton.setTitle("未連線", for: .normal)
