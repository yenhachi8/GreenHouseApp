...
class PublishViewController: UIViewController, UITextFieldDelegate {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        tabBarItem = UITabBarItem(title: "發佈", image: UIImage(named: "Publish.png"), tag: 1)
    }
    ...
    func updateUIState() {
        DispatchQueue.main.async() {
            if(connectionState == .Connected) {
                self.publishButton.setTitle("發佈", for: .normal)
                ...
                self.publishButton.setTitle("未連線", for: .normal)
